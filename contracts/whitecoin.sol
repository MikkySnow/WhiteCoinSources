...
contract WhiteCoin is ERC20  {
    ...
    mapping ( uint => address  ) public mintRequestReference;
    ...
    event Approval(address indexed owner, address indexed spender, uint value);
    event NewMintingRequest(uint256 indexed amount, uint256 indexed id, address indexed to);
   ...
   function removeMinter ( address _address ) onlyManager {
    ...
    function mintingRequest ( uint256 _amount , address to ) public onlyMinter returns (uint256) {
        uint256 id = mintRequestCount;
        mintRequestCount++;
        mintRequest[ mintRequestCount ] = _amount;
        mintRequestReference[ mintRequestCount ] = to;
        mintRequestor[ mintRequestCount ] = msg.sender;
        emit NewMintingRequest(_amount, id, to);
    }
    ...
    function mintWhitecoin( uint _mintRequest, uint8 _decision ) public onlyManager {
        require ( _mintRequest != 0 && _mintRequest <= mintRequestCount &&  mintRequestStatus [ _mintRequest ] == 0 && _decision != 0 && _decision < 3 );
        if ( _decision == 2 )  { mintRequestStatus [ _mintRequest ] = 2; return; } // Mint Declined
        // Updates manager balance
        balanceOf[msg.sender] = balanceOf[msg.sender].add(mintRequest[_mintRequest]);
        // Updates totalSupply
        totalSupply = totalSupply.add( mintRequest[ _mintRequest ] );
        // Updates status of request
        mintRequestStatus [ _mintRequest ] = 1;
        // Sends whitecoin to client
        transfer(mintRequestReference[_mintRequest], mintRequest[_mintRequest]);
